# -*- coding: utf-8 -*-
"""
Python Frontmatter: Parse and manage tasks with YAML frontmatter

I pulled this package and renamed "Posts" to "Tasks" because I
prefer that terminology. Credit to Chris Amico @eyeseast for 
...
def load(fd, encoding="utf-8", handler=None, **defaults):
    ...
    return a :py:class:`task <frontmatter.Task>`.

    .. doctest::

        >>> task = frontmatter.load('tests/yaml/hello-world.txt')
        >>> with open('tests/yaml/hello-world.txt') as f:
        ...     task = frontmatter.load(f)
...
def loads(text, encoding="utf-8", handler=None, **defaults):
    """
    Parse text (binary or unicode) and return a :py:class:`task <frontmatter.Task>`.

    .. doctest::

        >>> with open('tests/yaml/hello-world.txt') as f:
        ...     task = frontmatter.loads(f.read())

    """
    text = u(text, encoding)
    handler = handler or detect_format(text, handlers)
    metadata, content = parse(text, encoding, handler, **defaults)
    return Task(content, handler, **metadata)


def dump(task, fd, encoding="utf-8", handler=None, **kwargs):
    """
    Serialize :py:class:`task <frontmatter.Task>` to a string and write to a file-like object.
    ...
    """
    content = dumps(task, handler, **kwargs)
    if hasattr(fd, "write"):
...
def dumps(task, handler=None, **kwargs):
    """
    Serialize a :py:class:`task <frontmatter.Task>` to a string and return text.
    ...
    passed as an argument will override ``task.handler``, with
    ...
    ::
        ...
        >>> task = frontmatter.load('tests/yaml/hello-world.txt')
        >>> print(frontmatter.dumps(task)) # doctest: +NORMALIZE_WHITESPACE
        ---
        layout: task
        title: Hello, world!
        ---
    ...
    """
    if handler is None:
        handler = getattr(task, "handler", None) or YAMLHandler()

    return handler.format(task, **kwargs)


class Task(object):
    """
    A task contains content and metadata from Front Matter. This is what gets
    ...
    def to_dict(self):
        "Task as a dict, for serializing"
